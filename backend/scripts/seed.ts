 ...
 *
 * History generation:
 *   For each product, we generate 60 daily price points (daysAgo 0..59) using
 *   one of four patterns rotated by index (idx % 4):
 ...
 *     bucket 3 — recent drop   : flat around 1.20*currentPrice for daysAgo>=3,
 *                                drop to currentPrice for daysAgo<3 (genuine
 *                                flash sale, exercises the verdict path that
 *                                price_7d_ago is unambiguously above current).
 */
...
// 60 days exercises the chart's longer-window rendering and gives the cron a
// realistic baseline of historical points after first run. Still well under
// the API's PRICE_HISTORY_LIMIT (180) so /api/price returns the full window.
...
/**
 * Returns HISTORY_DAYS entries with daysAgo from 0..HISTORY_DAYS-1.
 * Pattern depends on `bucket`:
 *   0 stable     — flat around currentPrice ±2%
 *   1 real       — baseline ~25% above currentPrice for daysAgo>=7, lerp down
 *                  to currentPrice for daysAgo<7
 ...
 *   3 recent_drop — flat around currentPrice*1.20 for daysAgo>=3, lerp down
 *                   to currentPrice for daysAgo<3 (genuine flash sale within
 *                   the last 72h — exercises the verdict path where
 *                   price_7d_ago is clearly above current_price)
...
function generateHistory(
  ...
  for (let daysAgo = 0; daysAgo < HISTORY_DAYS; daysAgo++) {
    ...
    if (bucket === 0) {
      ...
      // recent_drop — genuine flash sale in the last 3 days
      const baseline = currentPrice * 1.2;
      if (daysAgo >= 3) {
        p = baseline * noise();
      } else {
        // lerp from currentPrice (at daysAgo=0) to baseline (at daysAgo=3)
        const t = (3 - daysAgo) / 3; // 1 at daysAgo=0, 0 at daysAgo=3
        const v = currentPrice + (baseline - currentPrice) * (1 - t);
        p = v * noise();
      }
...
async function main(): Promise<void> {
  ...
  lines.push(
    `-- Generated by scripts/seed.ts at ${new Date().toISOString()}. ` +
      `Up to ${QUERIES.length * LIMIT_PER_QUERY} products x ${HISTORY_DAYS} history points x 4 patterns (stable/real/inflated/recent_drop rotated by index).`,
  );
